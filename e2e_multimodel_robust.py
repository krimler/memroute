...
CONFIG = {
    "models": ["gpt-5.1", "gpt-3.5-turbo", "gpt-4o-mini"],
...
try:
    import tiktoken
    ENCODER = tiktoken.encoding_for_model("gpt-5.1")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] ✓ tiktoken loaded")
except Exception as e:
    ENCODER = None
    print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠ tiktoken failed: {e}")
...
def ask_llm(query: str, context: str, model: str, question_id: int) -> str:
...
Answer concisely:"""
    ...
    for attempt in range(CONFIG["max_retries"]):
        try:
            ...
            # Success log (only for slow responses)
            if elapsed > 5:
                log(f"  Q{question_id}: {elapsed:.1f}s (slow)")
            
            return result
            
        except Exception as e:
            wait_time = CONFIG["retry_backoff"] ** attempt
            log(f"  Q{question_id} ERROR (attempt {attempt+1}/{CONFIG['max_retries']}): {str(e)[:50]}")
...
def run_evaluation(resume: bool = True):
    ...
    if not USE_REAL_LLM:
        log("✗ Cannot run without API key")
        return None
    ...
    for model in CONFIG["models"]:
        ...
        for policy_name, policy_fn in policy_list:
            ...
            for q_idx, item in enumerate(questions):
                ...
                stores = policy_fn(q)
                context, tokens = retrieve_context(stores, memory)
                
                # Call LLM
                response = ask_llm(q["query"], context, model, q_idx)
